...
async def test_resolves_medium_term_signal_to_five_day_observation(db_session):
    ...
    try:
        ...
        assert result.market_data_expected_count == 1
        assert result.market_data_resolved_count == 1
        assert result.market_data_coverage_ratio == 1.0
    finally:
...
async def test_excludes_uncertain_horizon(db_session):
    ...
    try:
        ...
        assert result.market_data_expected_count == 0
        assert result.market_data_resolved_count == 0
        assert result.market_data_coverage_ratio is None
...
async def test_requires_entry_bar_strictly_after_signal(db_session):
    ...
    try:
        ...
        assert len(result.observations) == 1
        assert result.observations[0].forward_return_pct == pytest.approx(
            (110 / 105 - 1) * 100,
            rel=1e-6,
        )
    finally:
...
@pytest.mark.asyncio
async def test_reports_insufficient_market_data_coverage(db_session):
    instrument = await create_instrument(db_session)
    ...
    try:
        ...
        assert len(result.observations) == 1
        assert any("Market-data coverage" in note for note in result.notes)
    finally:
        await cleanup(db_session, instrument.id)
