...
class ModelMonitoringService:
    """Compute deterministic monitoring metrics from model output windows."""
    ...
    def build_report(
        ...
        *,
        ...
        probabilities = tuple(
            item.predicted_probability for item in request.predictions
        )
        ...
        prediction_drift = self._prediction_drift(
            request.reference_predictions,
            probabilities,
        )
        feature_drift = tuple(
            self._feature_drift(
                name,
                request.reference_features[name],
        ...
        if not request.reference_features or not request.current_features:
            notes.append(
                "Feature drift was not assessed because a reference and "
                "current feature window were not both supplied.",
            )

        if not request.reference_predictions:
            notes.append(
                "Prediction drift was not assessed because no reference "
                "prediction window was supplied.",
            )

        if not request.market_returns:
            notes.append(
                "Market regime was classified as unavailable because no "
                "market returns were supplied.",
            )
    ...
    def _calibration(
        ...
        ece = sum(
            ...
            / len(labels)
            * abs(
                mean(probability for probability, _ in items)
                - mean(label for _, label in items),
            )
            for items in bins
            if items
        )
    ...
    def _prediction_drift(
        ...
        if not reference:
            return DriftMetric(
                ...
                psi=None,
                drifted=False,
                notes=("A reference prediction window is required for PSI.",),
            )

        psi = _psi(
            _probability_histogram(reference),
            _probability_histogram(current),
        )
    ...
    def _feature_drift(
        ...
        psi = _psi(
            _histogram(reference, minimum=low, maximum=high),
            _histogram(current, minimum=low, maximum=high),
        )
...
def _probability_histogram(values: tuple[float, ...]) -> tuple[float, ...]:
    counts = [0] * 10
...
def _histogram(
    ...
    *,
    ...
    for value in values:
        index = min(
            max(
                int((value - minimum) / (maximum - minimum) * 10),
                0,
            ),
            9,
        )
...
def _psi(
    ...
    return sum(
        ...
        * math.log(current[index] / reference[index])
